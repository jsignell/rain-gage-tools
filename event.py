...
class Event:
    ...
    def __init__(self, df):
        ...
        **kwargs
        ...
        self.ll_cols = [col for col in self.df.columns if col not in cols]
        self.ll = self.df[ll_cols]
    ...
    def movie(self, vmin=None, vmax=None, cmap='gist_earth_r', latlon=True):
        ...
        **kwargs
        ...
        def animate(i):
            ax.set_title(df[[i+len(ll_cols)]].columns[0])
            scat = ax.scatter(x=x, y=y, cmap=cmap, c=df[[i+len(ll_cols)]], vmin=0, vmax=vmax, s=100)
    ...
    def krige(self, i=5, v=None, step=1, plot_v=False, plot_k=True, animated=False, **plot_kwargs):
        """
        ...
        **kwargs
        ...
        if plot_k and animated:
            ...
            self.plot_krige(i, k, rng, step=step, animated=animated, **plot_kwargs)
            return k
        return k

    def plot_krige(self, i, k, rng, step=1, animated=False, ax=None, cmap='gist_earth_r', vmin=None, vmax=None):
        df = self.df
        if not ax:
            fig, ax = plt.subplots()
        if not vmin:
